...
The frontend can show a preview, but it cannot grant ownership by itself. The user selects an approved catalog item. The server checks that it is published and Guardian-approved, checks the Coin price, confirms the user’s balance, writes the Coin transaction, and creates the entitlement. If the user already owns the item, the system returns the existing entitlement rather than charging again. Afterward, the client refreshes the entitlement list and applies only what the server confirms.
